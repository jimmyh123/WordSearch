...
import com.jimmyh123.wordsearch.util.Constants
import com.jimmyh123.wordsearch.util.Constants.MAX_NO_OF_TURNS
...
class GameViewModel @Inject constructor(
    ...
    private fun findUniqueAnswerIndex(): Int{
        ...
        if (!currentQuizList.isNullOrEmpty()){
            quizListSize = currentQuizList!!.size
            randomIndex = currentQuizList!!.indices.random()
            currentWord = currentQuizList!![randomIndex]?.answer
        }

        return if (
            usedAnswerList.contains(currentWord) &&
            quizListSize > usedAnswerList.size &&
            currentWord!=""
        ) {
